...
class QualityCalculator:
    ...
    def calculate_quality_score(self, image: np.ndarray, roi_coords: Optional[list] = None, progress_cb=None) -> Dict:
        """
        ...
        Args:
            ...
            progress_cb: Optional callable(fraction_0to1, label) for progress reporting
        ...
        if progress_cb: progress_cb(0.05, "Scoring gradient quality…")
        gradient_metrics = self._calculate_gradient_quality(analysis_region)
        if progress_cb: progress_cb(0.25, "Scoring contrast quality…")
        contrast_metrics = self._calculate_contrast_quality(analysis_region)
        if progress_cb: progress_cb(0.45, "Scoring entropy quality…")
        entropy_metrics = self._calculate_entropy_quality(analysis_region)
        if progress_cb: progress_cb(0.65, "Scoring pattern quality…")
        pattern_metrics = self._calculate_pattern_quality(analysis_region)
        if progress_cb: progress_cb(0.85, "Scoring noise quality…")
    ...
    def get_component_breakdown(self, image: np.ndarray, roi_coords: Optional[list] = None, progress_cb=None) -> dict:
        """
        Return a normalized per-metric breakdown that matches the overall score.

        Each component score is on a 0-100 scale. The five weighted_contribution
        values sum to the overall_score (within rounding).

        Returns:
            dict with keys: gradient, contrast, entropy, pattern, noise, overall_score, weights_source
        """
        result = self.calculate_quality_score(image, roi_coords, progress_cb=progress_cb)

        gm = result['gradient_metrics']
        cm = result['contrast_metrics']
        em = result['entropy_metrics']
        ...
        components = {
            'gradient': {
                ...
                'score': round(gm['score'] * 100, 1),
                'weighted_contribution': round(gm['score'] * 100 * self.weights['gradient'], 2),
                'sub_metrics': {
                    'MIG': round(gm.get('raw_mig', 0), 3),
                    'Ef': round(gm.get('raw_ef', 0), 3),
                    'distribution_bonus': round(gm.get('distribution_bonus', 0), 3),
                }
            },
            'contrast': {
                'weight': self.weights['contrast'],
                'score': round(cm['score'] * 100, 1),
                'weighted_contribution': round(cm['score'] * 100 * self.weights['contrast'], 2),
                'sub_metrics': {
                    'rms_contrast': round(cm.get('rms_contrast', 0), 4),
                    'michelson_contrast': round(cm.get('michelson_contrast', 0), 4),
                    'weber_contrast': round(cm.get('weber_contrast', 0), 4),
                    'local_contrast': round(cm.get('local_contrast', 0), 4),
                }
            },
            'entropy': {
                'weight': self.weights['entropy'],
                'score': round(em['score'] * 100, 1),
                'weighted_contribution': round(em['score'] * 100 * self.weights['entropy'], 2),
                'sub_metrics': {
                    'shannon_entropy': round(em.get('shannon_entropy', 0), 4),
                    'local_entropy_avg': round(em.get('local_entropy_avg', 0), 4),
                }
            },
            'pattern': {
                'weight': self.weights['pattern'],
                'score': round(pm['score'] * 100, 1),
                'weighted_contribution': round(pm['score'] * 100 * self.weights['pattern'], 2),
                'sub_metrics': {
                    'avg_speckle_diameter': round(pm.get('avg_speckle_diameter', 0), 2),
                    'speckle_density': round(pm.get('speckle_density', 0), 2),
                    'coverage': round(pm.get('coverage', 0), 4),
                    'pattern_uniformity': round(pm.get('pattern_uniformity', 0), 2),
                }
            },
            'noise': {
                'weight': self.weights['noise'],
                'score': round(nm['score'] * 100, 1),
                'weighted_contribution': round(nm['score'] * 100 * self.weights['noise'], 2),
                'sub_metrics': {
                    'snr_db': round(nm.get('snr_db', 0), 2),
                    'noise_std': round(nm.get('noise_std', 0), 4),
                    'signal_std': round(nm.get('signal_std', 0), 4),
                }
            },
        }
        ...
        return {
            **components,
            ...
            'weights_source': 'calculate_quality_score',
        }

    def calculate_subset_component_scores(self, subset: np.ndarray) -> dict:
        """
        ...
        try:
            if subset.size == 0 or subset.shape[0] < 3 or subset.shape[1] < 3:
                return {'gradient': 0.0, 'contrast': 0.0, 'entropy': 0.0, 'pattern': 0.5, 'noise': 0.5}
    ...
    def calculate_subset_all(self, subset: np.ndarray) -> Tuple[float, dict]:
        """
        ...
        try:
            ...
            logger.warning(f"Error in calculate_subset_all: {ex}")
            return 0.0, _default_comp

    def _calculate_fast_pattern_quality(self, gray: np.ndarray) -> float:
        """Fast pattern quality calculation for subset analysis."""
        ...
        try:
            ...
            if num_labels > 1:
                ...
                min_area = max(2, int(gray.size * 0.0001))
                max_area = int(gray.size * 0.01)
                valid_areas = areas[(areas >= min_area) & (areas <= max_area)]
                if len(valid_areas) > 0:
                    coverage = float(np.sum(valid_areas)) / gray.size
                    density = len(valid_areas) / gray.size * 1e6
                    coverage_score = 1.0 if 0.001 <= coverage <= 0.1 else 0.6
                    density_score = 1.0 if 50 <= density <= 5000 else 0.7
                    return min(1.0, coverage_score * 0.6 + density_score * 0.4)
                return 0.3
            return 0.2
        except Exception:
            return 0.5

    def _calculate_fast_noise_quality(self, gray: np.ndarray) -> float:
        """Fast noise quality calculation for subset analysis."""
        if gray.shape[0] < 5 or gray.shape[1] < 5:
            return 0.5
        try:
            denoised = cv2.GaussianBlur(gray, (5, 5), 1.0)
            noise_std = float(np.std(gray.astype(float) - denoised.astype(float)))
            signal_std = float(np.std(denoised))
            if noise_std > 0:
                snr = signal_std / noise_std
                snr_db = 20 * np.log10(snr) if snr > 0 else 0.0
            else:
                snr_db = 100.0
            return min(1.0, snr_db / 30.0) if snr_db > 0 else 0.0
        except Exception:
            return 0.5
    ...
    def calculate_live_analysis_quality_parallel(self, image: np.ndarray, grid_size: tuple = None) -> tuple:
        ...
        try:
            ...
            # Step 1: Calculate accurate overall score
            full_result = self.calculate_quality_score(gray)
