...
from DIC.ui.main_components.image_canvas import ImageCanvas
from DIC.ui.main_components.top_navigation import TopNavigationBar
# ROI selector now integrated into ImageCanvas
from DIC.ui.main_components.legend_panel import LegendPanel
from DIC.ui.main_components.results_popup import ResultsPopup
from DIC.ui.dialogs.help_dialog import HelpDialog
from DIC.ui.dialogs.screenshot_dialog import ScreenshotDialog

from DIC.models.application_state import ApplicationState
from DIC.models.analysis_result import AnalysisResult
from DIC.core.image_analyzer import ImageAnalyzer
from DIC.core.report_generator import ReportGenerator
from DIC.utils.file_operations import FileOperationsManager

from DIC.utils.constants import APP_CONFIG, get_theme_colors
from DIC.utils.modern_styling import ModernStyleManager
...
class DICQualityInspector:
    ...
    def _create_ui(self):
        ...
        nav_callbacks = {
            ...
            'show_metric_map': self.show_metric_map,
        }
    ...
    def _create_status_bar(self):
        """Create modern status bar with progress indicator."""
        colors = get_theme_colors()
        ...
        # Inner row: progress bar (left) + status label (right fills rest)
        inner = tk.Frame(self.status_container, bg=colors['panel_bg'])
        ...
        style.configure(
            ...
            background=colors.get('btn_primary', '#2563eb'),
            borderwidth=0,
            thickness=4,
        )
    ...
    def _on_analysis_progress(self, fraction: float, label: str):
        ...
        self.root.after(0, lambda: self._apply_progress(pct, label))

    def _apply_progress(self, pct: float, label: str):
        """Update progress bar and status label (must run on main thread)."""
    ...
    def toggle_theme(self):
        ...
        from DIC.utils.constants import set_theme
    ...
    def _refresh_ui_theme(self):
        ...
        # Re-show the currently selected metric map if active
        if self.state.has_analysis_result() and self.image_canvas.is_showing_quality_map():
            self.show_metric_map(self.current_metric)
    ...
    def _update_ui_state(self):
        ...
        # Build informative status messages based on available state data
        if state == 'no_image':
            msg = "Ready - Load an image or capture a screenshot to begin"
        elif state == 'image_loaded':
            img_info = self.state.get_image_info()
            if img_info:
                fname = img_info.get('filename') or ''
                fname = os.path.basename(fname) if fname else img_info.get('source', 'image')
                w, h = img_info.get('width', '?'), img_info.get('height', '?')
                msg = f"Loaded: {fname} ({w}x{h}px) - Select ROI or click Analyze"
            else:
                msg = "Image loaded - Select ROI or analyze full image"
        elif state == 'roi_selected':
            roi_info = self.state.get_roi_info()
            if roi_info:
                area = roi_info.get('area', 0)
                pct = roi_info.get('percentage', 0)
                msg = f"ROI selected ({area:.0f} px\u00b2, {pct:.1f}% of image) - Click Analyze to begin"
            else:
                msg = "ROI selected - Ready for analysis"
        elif state == 'analyzing':
            msg = "Analyzing..."  # Progress callback will overwrite this with step details
        elif state == 'analysis_complete':
            result = self.state.get_analysis_result()
            if result:
                assessment = result.get_quality_assessment() if hasattr(result, 'get_quality_assessment') else ''
                msg = f"Complete: {result.overall_score:.1f}/100 ({assessment}) - View results or load new image"
            else:
                msg = "Analysis complete - View results or adjust settings"
        else:
            msg = "Ready"
    ...
    def select_roi(self):
        ...
        if self.state.has_image():
            self.status_var.set("Draw ROI on image - click to add points, double-click or press Enter to finish")
    ...
    def _auto_show_quality_map(self):
        ...
        if self.state.has_analysis_result() and not self.image_canvas.is_showing_quality_map():
            self.current_metric = 'overall'
            if self.top_navigation:
                self.top_navigation.set_active_metric('overall')
    ...
    def show_metric_map(self, metric_name: str):
        """Display a specific component metric map on the canvas."""
        if not self.state.has_analysis_result():
            return
        ...
        if metric_name == 'overall':
            chosen_map = result.quality_map
        else:
            if result.metric_maps is None or metric_name not in result.metric_maps:
                # Graceful degradation: fall back to overall map
                logger.warning(f"Metric map '{metric_name}' not available, showing overall map")
                chosen_map = result.quality_map
                metric_name = 'overall'
            else:
                chosen_map = result.metric_maps[metric_name]

        self.current_metric = metric_name

        # Ensure map is visible
        if not self.image_canvas.is_showing_quality_map():
            self.control_panel.set_quality_map_active(True)

        self.image_canvas.show_quality_map(chosen_map, spectrum_type)
        self.legend_panel.show_legend(spectrum_type)

        # Keep selector button in sync (in case called programmatically)
        if self.top_navigation:
            self.top_navigation.set_active_metric(metric_name)
    ...
    def start_spinview_capture(self):
        ...
        try:
            ...
            if not self.spinview_mode:
                from DIC.ui.live_analyze.spinview_capture_mode import SpinViewCaptureMode
